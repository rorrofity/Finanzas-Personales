...
0. ✅ **Sincronización Automática Programada + Push (Epic 13) — COMPLETADA Y DESPLEGADA** (2026-07-19): scheduler backend (node-cron, 13:00 y 22:00 America/Santiago) que dispara la sync de N8N automáticamente (opt-in por dueño, `auto_sync_enabled`) y envía notificación Web Push (VAPID) cuando trae transacciones nuevas, para recordar categorizar. Suscripciones push por persona (no por espacio). Verificado en producción con cuenta temporal. Pendiente manual: Rodrigo activa desde Settings → Automatización en su teléfono real. Ver walkthrough.md Parte 4.
1. ✅ **Épica PWA completada y desplegada a producción** (2026-07-18).
2. ✅ **Rediseño del Dashboard (Epic 12) — COMPLETADA Y DESPLEGADA** (2026-07-18): endpoint `/api/dashboard/overview` con 7 KPIs (balance, gastos+Δ, tasa ahorro, burn rate, disponible hoy, compromisos, top categorías); sistema de diseño compacto (`src/components/ui/`: StatCard, TrendDelta, CategoryBar, SectionCard, ChartTabs); dashboard con stat-cards 2×2, compromisos colapsables, top categorías con barras y gráficos bajo tabs; totales compactos en las 4 páginas de transacciones. Ver walkthrough.md Parte 3.
2b. ✅ **Espacio Compartido del Hogar (Epic 11) — COMPLETADA Y DESPLEGADA** (2026-07-18): el dueño invita (in-app) hasta 2 miembros con permisos granulares (ver / crear+editar / eliminar) activables al instante; modelo ACL sobre la cuenta del dueño (`space_members` + header `X-Space-Owner` + middleware `resolveSpace`, ACL-001), auditoría `created_by/updated_by`, sync N8N y config exclusivos del dueño, switcher de espacio en la UI, caché offline llaveado por espacio. Verificada funcionalmente en producción. Pendiente: que Rodrigo invite a su pareja con sus cuentas reales. Ver walkthrough.md Parte 2.
